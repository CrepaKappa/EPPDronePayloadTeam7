...
% Open relevant files
model = femodel(AnalysisType="structuralStatic", Geometry="Drone Project CAD\DroneArmB.STEP");
...
model.FaceBC(9) = faceBC(Constraint="fixed"); % Fixed to drone body
...
model.FaceLoad(14) = faceLoad("SurfaceTraction",[0,0,-weightTraction]); % Weight of motor
model.FaceLoad(17) = faceLoad("SurfaceTraction",[0,0,thrustTraction]); % Force of propeller pull
model.FaceLoad(7) = faceLoad("Gravity",[0,0,-g]);
...
massModel = createpde;
importGeometry(massModel,"Drone Project CAD\DroneArmB.STEP");
