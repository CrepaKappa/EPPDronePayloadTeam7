...
% Open relevant files
model = femodel(AnalysisType="structuralStatic", Geometry="Drone Project CAD\DroneArmC.STEP");
...
model.FaceBC(10) = faceBC(Constraint="fixed"); % Fixed to drone body
...
model.FaceLoad(19) = faceLoad("SurfaceTraction",[0,0,-weightTraction]); % Weight of motor
model.FaceLoad(16) = faceLoad("SurfaceTraction",[0,0,thrustTraction]); % Force of propeller pull
model.FaceLoad(11) = faceLoad("Gravity",[0,0,-g]);
...
massModel = createpde;
importGeometry(massModel,"Drone Project CAD\DroneArmC.STEP");
